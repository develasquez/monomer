...
monomer = {
		...
		__setAspect : function(){
            ...
            _content={
	        	width :window.innerWidth,
	            height :window.innerHeight - _header.height - _footer.height - _tabBar.height - 5
	        }
			...
			$(".floating-down-right").css({"top": _window.height -  _footer.height - (em * 4) - 43 , "left" : _window.width - (em * 2) - 56});
			
			$(".floating-down-right-multiple.collapsed").css({"top": parseInt($(".floating-down-right-multiple.collapsed").parent().css("height")) - (em * 2) , "left" : $(".floating-down-right-multiple.collapsed").parent().width() - (em * 2) });
			$(".floating-down-right-multiple.expanded").css({"top": parseInt($(".floating-down-right-multiple.expanded").parent().css("height"))  - (em * 7.5), "left" : $(".floating-down-right-multiple.expanded").parent().width() - (em * 7.5) });
		...
		__init : function () {
			...
			$(".expand-search").unbind("mousedown");
			$(".expand-search").on("mousedown",function () {
			...
			$(".floating-down-right-multiple .action").unbind("click")
			$(".floating-down-right-multiple .action").on("click",function () {
				fDRMultiple = $(this).parent(".floating-down-right-multiple");
				$(fDRMultiple).children("button, a").toggleClass("floating-displayed");
				if($(fDRMultiple).hasClass("collapsed")){
					$(fDRMultiple).removeClass("collapsed");
					$(fDRMultiple).addClass("expanded");
					
					parentContent = $(fDRMultiple).parent(".content");
					$(fDRMultiple).css({"top": parseInt($(parentContent).css("height")) - (em * 7.5) , "left" : $(parentContent).width() - (em * 7.5) });
				}else{
					$(fDRMultiple).removeClass("expanded");
					$(fDRMultiple).addClass("collapsed");
					parentContent = $(fDRMultiple).parent(".content");
					
					$(fDRMultiple).css({"top": parseInt($(parentContent).css("height")) - (em * 2), "left" : $(parentContent).width() - (em * 2) });
				}
				actionIcon = $(fDRMultiple).find(".action i");
				if(actionIcon.hasClass("icon-share")){
					monomer.icon.to(actionIcon,"icon-share","icon-times");
				}else{
					monomer.icon.to(actionIcon,"icon-times","icon-share");	
				}
			})
		}
        };
	...
	$(function () {
		try{
		 ...
		 $(".tabBar li.tab").on("click",function(){
		   ...
		   ul.find(".selectedTab").css("margin-left", parseFloat(li.css("width")) * li.index() + "px")
